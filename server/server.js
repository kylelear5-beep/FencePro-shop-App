// server.js — FencePro Shop Assistant (Unified Build)
...
const uploadDir = path.join(ROOT_DIR, 'public', 'docs');
...
// Always serve public/ for docs and static assets
...
app.use((req, res, next) => {
  if (req.method === 'GET' && !req.path.startsWith('/api/')) {
    // In production, serve the Vite-built index.html
    const indexPath = path.join(ROOT_DIR, 'dist', 'index.html');
    if (fs.existsSync(indexPath)) {
      res.sendFile(indexPath);
    } else {
      // Fallback to public/index.html (legacy or dev)
